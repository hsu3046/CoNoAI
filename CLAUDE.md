...
- **마이크 채점 시간축** (`SingingTracker`): 마이크 샘플의 호스트 시각을 "그 순간 들리던 출력 스트림 초"로 옮긴다 (`HeardClock` = displayPosition 과 같은 계산). 재생이 멈춤·채우는 중·이동 음소거면 채점하지 않는다. 검출기는 원곡용과 **다른 SwiftF0 인스턴스** (모델 객체는 한 스레드만), 미리 보기 3 프레임(48 ms).
- **곡 끝내기 → 다음 곡** (`endSong` / `advanceToNextSong`): 끝내는 순간 원곡 앱을 **다음 곡 0초에 멈춰 둔다** (`cueNextSong`). 재생 때 넘기면 그 사이 CoNo 를 끄거나 음악 앱에서 재생할 때 끝낸 곡이 되살아난다. 끝낸 곡의 지연 버퍼는 버리지 않고 음소거로 흘려보낸다 (스트림 시각 유지) — "지금 캡처 위치" 까지 가리면 넘기며 새어 든 소리도 함께 가려진다.
- **스피커 누설 판정에 "원곡과 같은 음이면 스피커 소리" 규칙 금지**: 정확히 부른 음이 버려진다. 누설은 출력 레벨(`outputLevels`) 대비 마이크 레벨로만 거른다 (`LeakageGate`).
